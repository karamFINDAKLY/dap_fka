...
public class GmailService {
    ...
    /** The only instance of Gmail that will be used (Singleton). */
     ...
     */
    private Gmail getService(String userKeyValue) throws GeneralSecurityException, IOException {

        // Build a new authorized API client service.
        final NetHttpTransport httpTransport = GoogleNetHttpTransport.newTrustedTransport();
        Gmail service = new Gmail.Builder(httpTransport, JSON_FACTORY,
                Utils.getCredentials(httpTransport, userKeyValue)).setApplicationName(APPLICATION_NAME).build();
     ...
     * @throws GeneralSecurityException in case there's a security failure
     */
    public Integer getNbUnreadEmail(String uKey) throws IOException, GeneralSecurityException {

        String user = "me";

        // Alg to show number of unread emails
        ListMessagesResponse allMessages = getService(uKey).users().messages().list(user).setQ("is:unread in:inbox")
                .execute();
